...
function shortNonJsonError(status: number, body: string): Error {
  if (isProbablyHtml(body)) {
    ...
    const devHint =
      ' Local dev: `vite.config.ts` no longer forwards `/api/stripe` to the generic `/api` host (that returned HTML). Set `VITE_STRIPE_API_PROXY_TARGET` to a JSON backend, or `VITE_STRIPE_ADMIN_DEV_MOCK=1` for stubs (`src/api/stripe/admin.ts`). This request used `fetchStripeJson` (`/api/stripe`).';
    const msg = router
