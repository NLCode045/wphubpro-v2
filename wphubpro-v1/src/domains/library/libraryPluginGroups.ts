...
export function getItemsForGrouping(item: LibraryItem, libraryItems: LibraryItem[]): LibraryItem[] {
  if (item.type === LibraryItemType.Theme) {
    const tk = getThemeGroupKey(item);
    return libraryItems.filter(
      (i) => i.type === LibraryItemType.Theme && getThemeGroupKey(i) === tk,
    );
  }
  return libraryItems.filter(
    (i) =>
      i.type === LibraryItemType.Plugin &&
      (i.wpSlug?.toLowerCase() === getPluginSlug(item).toLowerCase() || getPluginSlug(i) === getPluginSlug(item)),
  );
}
...
export function pluginGroupSlugsWithOfficial(
  ...
  for (const g of pluginGroups) {
    const hasOfficial = g.items.some(
      (i) => i.type === LibraryItemType.Plugin && i.source === LibraryItemSource.Official,
    );
...
export function getPluginGroups(pluginItems: LibraryItem[]): { slug: string; displayName: string; items: LibraryItem[] }[] {
  const seen = new Set<string>();
  ...
  for (const item of pluginItems) {
    ...
    const displayName =
      items.find((i) => i.source === LibraryItemSource.Official)?.name ?? items[0]?.name ?? slug;
