...
import { expandLibraryDocumentToItems } from './libraryDocument';

/** First expanded row (legacy callers); prefer `expandLibraryDocumentToItems` for multi-version docs. */
...
const toCollectionMember = (raw: Record<string, unknown>): LibraryCollectionMember | null => {
  ...
  const type = typeStr === LibraryItemType.Theme ? LibraryItemType.Theme : LibraryItemType.Plugin;
...
function parseMemberPreferencesJson(raw: unknown): Record<string, LibraryFamilyMemberPreference> | undefined {
  ...
  try {
    ...
    for (const [k, v] of Object.entries(parsed as Record<string, unknown>)) {
      ...
      if (!slug) continue;
      if (v && typeof v === 'object' && !Array.isArray(v) && typeof (v as { versionKey?: unknown }).versionKey === 'string') {
...
export const mapLibraryFamilyDocument = (doc: Record<string, any>): LibraryFamily => {
...
export const mapLibraryCollectionDocument = (doc: Record<string, any>): LibraryCollection => {
